...
for projeto, aba in zip(PROJETOS, abas):
    with aba:
        ...
        sla_mensal['fora_sla'] = 100 - sla_mensal['percentual']
        sla_mensal['cor'] = sla_mensal['percentual'].apply(lambda x: 'green' if x >= sla_meta else 'red')

        fig_sla = go.Figure()
        fig_sla.add_bar(x=sla_mensal['mes_str'], y=sla_mensal['percentual'], name='Dentro SLA', marker_color='green')
        fig_sla.add_bar(x=sla_mensal['mes_str'], y=sla_mensal['fora_sla'], name='Fora SLA', marker_color='red')
        fig_sla.update_layout(barmode='stack', yaxis_title='%', xaxis_title='Mês', title=f'SLA Mensal ({sla_meta}%)')
