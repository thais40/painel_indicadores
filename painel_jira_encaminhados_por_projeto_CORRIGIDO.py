...
st.title('Painel de Encaminhamentos - INT e TDS (Corrigido)')
...
df['escalado_n3_valor'] = df['escalado_n3'].apply(lambda x: x.get('value') if isinstance(x, dict) else None)
...
for projeto_nome, aba in zip(['INT', 'TDS'], [aba_int, aba_tds]):
    with aba:
        ...
        with col1:
            encaminhados_produto = df_mes['status'].str.contains("Produto", case=False, na=False)
            st.metric("Encaminhados Produto", encaminhados_produto.sum())
        with col2:
            escalado_sim = df_mes['escalado_n3_valor'] == "Sim"
            st.metric("Encaminhados N3", escalado_sim.sum())
