...
CAMPO_ORIGEM = "customfield_13628"  # Origem do problema
        ...
        if projeto == "TDS":
            with st.expander("📱 APP NE — Origem do problema", expanded=False):
                ...
                if df_app.empty:
                    ...
                    df_app["origem_nome"] = df_app["origem"].apply(lambda x: x.get("value") if isinstance(x, dict) else str(x))
                    total_app = len(df_app)
                    contagem = df_app["origem_nome"].value_counts().to_dict()
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Total", total_app)
                    c2.metric("APP NE", contagem.get("APP NE", 0))
                    c3.metric("APP EN", contagem.get("APP EN", 0))

                    serie = df_app.groupby([df_app["mes_created"].dt.strftime("%b/%Y"), "origem_nome"]).size().reset_index(name="Qtd")
                    if serie.empty:
                        st.info("Sem série mensal para exibir.")
                    else:
                        fig_app = px.bar(
                            serie,
                            x="mes_created",
                        ...
                        st.dataframe(df_app[["key","created","assunto_nome","origem_nome","status"]],
